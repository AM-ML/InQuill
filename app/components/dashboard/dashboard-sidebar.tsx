...
export function DashboardSidebar() {
  ...
  const isAdmin =
    user?.role?.toLowerCase() === USER_ROLES.ADMIN.toLowerCase() ||
    user?.role?.toLowerCase() === USER_ROLES.OWNER.toLowerCase();
  ...
  const navigationItems = [
    ...
    // Admin Panel link that only appears for admins
    ...(isAdmin
      ? [
          {
            title: "Admin Panel",
  ...
  return (
    <Sidebar className="border-r border-gray-200 dark:border-gray-800 h-min">
      <SidebarContent
        className={`${isExpanded ? "px-4" : "px-2"}`}
