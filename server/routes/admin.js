...
router.get('/users', auth, authorize('admin', 'owner'), async (req, res) => {
  try {
    ...
    const formattedUsers = users.map(user => ({
      ...
      lastLogin: user.createdAt, // Placeholder as we don't track last login yet
      avatar: user.avatar || ''
    }));
...
router.get('/stats', auth, authorize('admin', 'owner'), async (req, res) => {
  try {
    ...
    const pendingArticles = await Article.countDocuments({ status: 'draft' });
    ...
    // Get views for the current month
    const currentMonthStart = new Date();
    currentMonthStart.setDate(1);
    currentMonthStart.setHours(0, 0, 0, 0);
    
    // This is mocked as we don't track views by date in this schema
    const viewsThisMonth = Math.round(totalViews * 0.3); // Assume 30% of views are from this month
    ...
    // For engagement data, we'd ideally track this over time
    // Here we're creating mock data based on article metrics
    const userEngagement = {
      views: viewsThisMonth,
      likes: Math.round(viewsThisMonth * 0.15), // Assume 15% of views result in likes
      comments: Math.round(viewsThisMonth * 0.05), // Assume 5% of views result in comments
      shares: Math.round(viewsThisMonth * 0.02), // Assume 2% of views result in shares
    };
    
    // Newsletter subscribers (mocked since we don't have a separate subscribers model)
    ...
    res.json({
      ...
      views: {
        total: totalViews,
        monthly: viewsThisMonth
      },
      engagement: userEngagement,
      contentDistribution: categories,
